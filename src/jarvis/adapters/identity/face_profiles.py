"""Atomic local storage for derived face embeddings."""
...
class JsonFaceProfileStore:
    """Store bounded normalized vectors in a user-editable local JSON file."""
    ...
    def profiles(self) -> dict[str, list[list[float]]]:
        """Load and validate profiles, returning an isolated copy."""
        ...
        try:
            ...
            raise IdentityError("The local face profile store is invalid.") from error

    def add_sample(self, name: str, embedding: list[float]) -> None:
        """Append one sample and replace the file atomically."""
        normalized_name = name.strip()
        if not normalized_name or not embedding:
            raise ValueError("A name and face embedding are required")
